...
export async function searchWithKetch(
  ...
  const output = await runKetch(executable, [
    "search", query, `--random=exa,ddg`, "--limit", String(maxResults), "--json",
  ], undefined, 15_000);
...
function runKetch(executable: string, args: string[], input: string | undefined, timeoutMs: number): Promise<string> {
  return new Promise((resolve, reject) => {
    const child = spawn(executable, args, { env: safeEnvironment(), stdio: ["pipe", "pipe", "pipe"] });
    const output: Buffer[] = [];
    const errors: Buffer[] = [];
    let bytes = 0;
